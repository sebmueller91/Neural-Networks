function Quadratic
  ...
  labels = ((-4*((x(1,:)-0.5).^2)+0.7) - x(2,:) > 0) + 1;
  for i=1:sum(x,2)
    y(labels(i),i) = 1;  
  end
  ...
  % Train the network
  [w1,b1,w2,b2,train_percentages,test_percentages] = TrainTwoLayerNN(s0,s1,s2, x_train, y_train, labels_train, x_test, labels_test, 1);
